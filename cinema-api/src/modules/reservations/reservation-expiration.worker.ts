...
export class ReservationExpirationWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = getLogger(ReservationExpirationWorker.name);
  ...
  constructor(
    ...
    this.logger.info(`Redis client status: ${client.status || 'unknown'}`);
  }
  ...
  async run() {
    ...
    await this.prisma.$transaction(async tx => {
      const expired: { id: string }[] = await tx.$queryRaw`
          SELECT id
          FROM "Reservation"
          WHERE status = 'ACTIVE'
            AND "expiresAt" < ${now}
      ...
      for (const { id } of expired) {
        ...
        this.logger.info(`Reservation expired: ${id}`);

        // publish event after transaction changes
        try {
          void this.messaging.publish('RESERVATION_EXPIRED', {
            reservationId: id,
            sessionId: res?.sessionId,
            seatIds: res?.seats?.map(s => s.seatId) || [],
          });
        } catch (err) {
          this.logger.error(`Failed publishing RESERVATION_EXPIRED: ${err}`);
        }
      }
    });
  }
  ...
  private async expireReservation(reservationId: string) {
    await this.prisma.$transaction(async tx => {
      ...
      this.logger.info(`Reservation expired: ${reservationId}`);
    });
  }
}
